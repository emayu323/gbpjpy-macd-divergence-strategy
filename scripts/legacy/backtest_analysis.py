...
def load_and_preprocess_data():
    ...
    print(f"1時間足データ: {len(h1_data):,} 行")
    print(f"5分足データ: {len(m5_data):,} 行")
...
def run_optimization(m5_data, h1_data):
    ...
    filter_combinations = [
        ...
        # フィルター2: ヒドゥンダイバージェンス追加
        {
            'name': '1時間足ヒドゥンダイバージェンス',
            'h1_rci_aligned': False,
            'h1_hidden_div': True,
            'm5_ema_divergence': False,
            'trigger_macd_div': True,
    ...
    for i, filters in enumerate(filter_combinations, 1):
        ...
        # スコア計算（勝率50%以上かつプロフィットファクター高いものを優先）
        score = 0
...
def main():
    ...
    print("\n")
    print("╔" + "=" * 58 + "╗")
    print("║" + " " * 10 + "GBPJPY トレード戦略バックテスト" + " " * 17 + "║")
    print("╚" + "=" * 58 + "╝")
    print("\n")
    ...
    if best_filters['h1_rci_aligned']:
        print(f"  - RCI 3本とも同方向")
    if best_filters['h1_hidden_div']:
        print(f"  - ヒドゥンダイバージェンス発生")

    print("\n5分足:")
    if best_filters['trigger_macd_div']:
        print(f"  - MACDダイバージェンス発生")
    if best_filters['trigger_rci_reversal']:
        print(f"  - RCI反転（±80ラインブレイク）")
    if best_filters['trigger_zigzag_break']:
        print(f"  - ZigZag短期の方向転換")
    if best_filters['m5_ema_divergence']:
    ...
    print(f"\n{'順位':<4} {'戦略名':<30} {'勝率':<8} {'PF':<8} {'純利益':<12}")
    print("-" * 70)

    for rank, result in enumerate(all_results, 1):
        perf = result['performance']
        name = result['filters']['name']
        print(f"{rank:<4} {name:<30} {perf['win_rate']:>6.2f}% {perf['profit_factor']:>6.2f} {perf['net_profit_pips']:>10.2f} pips")
