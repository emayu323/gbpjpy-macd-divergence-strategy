...
from indicators_v14 import (
    ...
    check_5m_entry_long_v14, check_5m_entry_short_v14
)
...
class BacktestEngineV14:
    """System Ver14 バックテストエンジン - 1H RCI短期±60条件追加"""
    ...
    def _check_1h_rci(self, data_1h: pd.Series, direction: str) -> bool:
    ...
    def _check_5m_entry(self, idx: int, direction: str) -> bool:
        """5分足のエントリートリガーをチェック（Ver14: RCI短期 + 5M パーフェクトオーダー）"""
        if idx == 0:
            return False
    ...
    def run_backtest(self) -> pd.DataFrame:
        ...
        for idx in range(len(self.df_5m)):
            ...
            if self.current_position is None:
                ...
                # 売りエントリーチェック（1H PO下降 + 1H RCI≥+60 + ダイバージェンス）
