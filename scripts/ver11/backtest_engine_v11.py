...
from indicators_v11 import (
    ...
    check_5m_entry_long_v11, check_5m_entry_short_v11
)
...
class BacktestEngineV11:
    """System Ver11 バックテストエンジン - 5分足パーフェクトオーダー追加、RCI中期削除"""
    ...
    def _update_divergences(self, current_time: pd.Timestamp, trend_4h: str):
        ...
        # Ver11: ヒドゥンダイバージェンスのみ検出（4Hトレンド条件なし）
    ...
    def _check_5m_entry(self, idx: int, direction: str) -> bool:
        """5分足のエントリートリガーをチェック（Ver11: RCI短期 + 5M パーフェクトオーダー）"""
        if idx == 0:
            return False
    ...
    def run_backtest(self) -> pd.DataFrame:
        ...
        for idx in range(len(self.df_5m)):
            ...
            if self.current_position is None:
                ...
                # Ver11: 4H 50EMA + 1Hパーフェクトオーダー + ヒドゥンダイバージェンスのみ
                if trend_4h == 'none':
                    continue
