"""
System Ver11 - MACD Divergence Strategy バックテスト実行スクリプト
Ver11: 4H環境認識なし + 1Hパーフェクトオーダー + ヒドゥンダイバージェンスのみ
"""
...
from backtest_engine_v11 import BacktestEngineV11
...
def print_statistics(stats: dict):
    """統計情報を表示"""
    print("\n" + "=" * 60)
    print("System Ver11 - MACD Divergence Strategy")
    print("バックテスト結果")
    print("=" * 60)
...
def save_results(df_trades: pd.DataFrame, df_divergences: pd.DataFrame, stats: dict,
    ...
    if df_monthly is not None and not df_monthly.empty:
        monthly_file = f"{config.RESULTS_DIR}/monthly_summary_v11.csv"
    ...
    if df_yearly is not None and not df_yearly.empty:
        yearly_file = f"{config.RESULTS_DIR}/yearly_summary_v11.csv"
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("System Ver11 - MACD Divergence Strategy\n")
        f.write("バックテストレポート\n")
        f.write("=" * 60 + "\n")
...
def main():
    ...
    # バックテスト実行
    print("\n" + "=" * 60)
    engine = BacktestEngineV11(df_5m, df_1h, df_4h)
