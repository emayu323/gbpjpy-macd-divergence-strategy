"""
System Ver18 - バックテストエンジン
Ver18: 5分足RCI±70反転 + 5M PO + 1H RCI±60 + 暫定ピボットDiv（1H POなし）
"""
...
class BacktestEngineV18:
    ...
    def run_backtest(self) -> pd.DataFrame:
        print("バックテスト開始...")
        print(f"Ver18: 1H PO={config.USE_1H_PO}, 1H RCI={config.USE_1H_RCI}")
        ...
        for idx in range(len(self.df_5m)):
            ...
            if self.current_position is None and self._is_trading_hours(current_time):
                ...
                po_ok_long = (po_1h == 'uptrend') if config.USE_1H_PO else True
                if po_ok_long and self._check_1h_rci(data_1h, 'long') and self._get_active_setup(current_time, 'long'):
                    if self._check_5m_entry(idx, 'long'):
                        self._open_position(idx, 'long')

                po_ok_short = (po_1h == 'downtrend') if config.USE_1H_PO else True
                if po_ok_short and self._check_1h_rci(data_1h, 'short') and self._get_active_setup(current_time, 'short'):
