"""
System Ver17 - バックテスト実行スクリプト
Ver17: 1H POあり / 1H RCIなし
"""
...
from backtest_engine_v17 import BacktestEngineV17
...
def save_results(engine, trades_df):
    ...
    if not trades_df.empty:
        trades_df.to_csv(results_dir / "trades_v17.csv", index=False)
    div_df = engine.get_divergences_dataframe()
    if not div_df.empty:
        div_df.to_csv(results_dir / "divergences_v17.csv", index=False)

    stats = engine.get_statistics()
    yearly = engine.get_yearly_summary()
    with open(results_dir / "backtest_report_v17.txt", 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("System Ver17 - MACD Divergence Strategy\n")
        f.write("1H POあり / 1H RCIなし\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    print("=" * 60)
    print("Ver17: 1H POあり / 1H RCIなし")
    print("=" * 60)
    df_5m, df_1h, df_4h = load_data()
    engine = BacktestEngineV17(df_5m, df_1h, df_4h)
